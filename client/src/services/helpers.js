...
export const isMetamaskApproved = async () => {
  ...
  const acc = await window._web3.eth.getAccounts()
  return !!acc.length;
...
export function checkProvider() {
  if (window.ethereum) {
    ...
    window._web3 = new Web3(new Web3.providers.HttpProvider(clientConfig.rpcHttpProvider));
...
export const inbrowserProviderName = () => {
  if (window._web3.currentProvider.isMetaMask) return 'Metamask';
  if (window._web3.currentProvider.isStatus) return 'Status';
  if (window._web3.currentProvider.isTrust) return 'Trust';
  if (window._web3.currentProvider.isToshi) return 'Coinbase';
  return 'Other'
};

export function getNetwork() {
  return new Promise((resolve, reject) => {
    window._web3.eth.net.getId((err, data) => {
      if (err) {
        return reject(err);
      }
      resolve(data);
...
export function getAccounts() {
  return new Promise((resolve, reject) => {
    window._web3.eth.getAccounts((err, data) => {
      if (err) {
        return reject(err);
      }
      resolve(data[0]);
