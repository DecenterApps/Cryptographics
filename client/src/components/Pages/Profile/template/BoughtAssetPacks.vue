    ...
    <div>
        <div class="asset-packs">
            ...
            <div v-if="assetPacks === false">
              <p>You haven't bought any asset packs yet. Go to <router-link to="/asset-pack-market">market</router-link> to buy asset packs.</p>
            </div>
        </div>
        <pagination
...
<script>
  ...
  export default {
    ...
    asyncComputed: {
      assetPacks: {
        async get () {
          let pageAssetPacksIds = await getPaginatedBoughtAssetPacks(1, this.showPerPage, this.userAddress);
