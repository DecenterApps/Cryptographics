...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      const filteredPacks = this.selectedAssetPacks.filter(item => {
        return !(
          this.createdPacksIDs.findIndex(
            id => parseInt(id, 10) === parseInt(item.id, 10)
          ) >= 0 ||
          this.boughtPacksIDs.findIndex(
            id => parseInt(id, 10) === parseInt(item.id, 10)
          ) >= 0
        );
      });
      return filteredPacks.reduce(
        (acc, item) =>
          Decimal(acc)
            .plus(item.price)
            .toString(),
  ...
  methods: {
    ...
    isSelected(asset) {
      return (
        this.selectedAssetPacks.findIndex(
          item => parseInt(item.id) === parseInt(asset.id)
        ) >= 0
      );
    },
